...
public class TreeBuild {
	...
	public Expression build(String postfixexpr, ExpressionFactory expfact) {
		...
		// NECESSITE JAVA 8 CAR LES VERSIONS ANTERIEURS N'ONT PAS LE MEME
		// COMPORTEMENT SUR CETTE FONCTION !!!!!
		String[] tokenarray = postfixexpr.split("\\s+");
		Expression racine;

		// si il n'y a qu'un element a evaluer
		if (tokenarray.length == 1) {
			racine = expfact.getExpression(tokenarray[0]);
		} else {
			// on construit la racine de l'abre avec le premier token
			racine = expfact.getExpression(tokenarray[tokenarray.length - 1]);
			...
			for (int i = tokenarray.length - 2; i >= 0; i--) {
				...
				if (lastinserted.peek().isOperator()) {
					 ...
					 * premier noeud sans fils gauche (on insere pas dans le
					 * dernier noeud, mais a partir de sont pere)
					 */
					// on depile la derniere insertion
					lastinserted.pop();
					boolean trouve = false;
					// tant qu'on a pas trouve ou inserer et que la pile
					// contient
					// bien des operators
					while (!trouve && lastinserted.peek().isOperator()) {
						...
						if (tmpexp.getLeftSide() == null) {
							// on insere l'expression courante en fils gauche et
							// on
